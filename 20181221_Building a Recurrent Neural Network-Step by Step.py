...
def rnn_cell_backward(da_next, cache):
    ...
    Arguments:
    da_next -- Gradient of loss with respect to next hidden state, of shape (n_a, m)
    ...
    # 要先算\partial J / \partial a * a'，再根據chain rule處理剩餘項
    dtanh = da_next * (1 - a_next ** 2) # n_a * m
    
    # compute the gradient of the loss with respect to Wax (≈2 lines)
    dxt = np.dot(Wax.T, dtanh) # (n_x, n_a) * (n_a, n_m) = (n_x, n_m)
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    dbf = dft.sum(axis = 1, keepdims = True)
    dbi = dit.sum(axis = 1, keepdims = True)
    dbc = dcct.sum(axis = 1, keepdims = True)
    dbo = dot.sum(axis = 1, keepdims = True)
