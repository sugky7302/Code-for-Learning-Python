...
def compute_content_cost(a_C, a_G):
    ...
    a_C_unrolled = tf.transpose(tf.reshape(a_C, [n_W * n_H, n_C]))
    a_G_unrolled = tf.transpose(tf.reshape(a_G, [n_W * n_H, n_C]))
    
    # compute the cost with tensorflow (≈1 line)
    # tf.math已經過時了
    # tf.squared_difference = (x - y)^2，也可以用tf.square(tf.subtract)取代
    J_content = tf.reduce_sum(tf.squared_difference(a_C_unrolled, a_G_unrolled)) / (4 * n_W * n_H * n_C)
    ### END CODE HERE ###
...
def compute_layer_style_cost(a_S, a_G):
    ...
    # reshape要按照順序，不能隨便設定，不然cost function的平方會算出不同值
