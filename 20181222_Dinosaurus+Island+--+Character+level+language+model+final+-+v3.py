...
def sample(parameters, char_to_ix, seed):
    ...
    while (idx != newline_character and counter != 50):
        ...
        x = np.zeros((vocab_size, 1)) # 初始化one-hot vector
        x[idx] = 1 #根據y的機率，找出機率最大值相對應的索引，把x的該位置設定成1，成功建立one-hot vector
...
def model(data, ix_to_char, char_to_ix, num_iterations = 35000, n_a = 50, dino_names = 7, vocab_size = 27):
    ...
    examples = [x.lower().strip() for x in examples] # 移除頭尾是小寫的字母
    
    # Shuffle list of all dinosaur names
    np.random.seed(0)
    np.random.shuffle(examples) # 隨機排序
    ...
    for j in range(num_iterations):
        ...
        index = j % len(examples)
        X = [None] + [char_to_ix[ch] for ch in examples[index]] # 讀取字串對應的index，加[None]是因為
        Y = X[1:] + [char_to_ix["\n"]]
